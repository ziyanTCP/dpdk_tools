...
uint8_t * send_ip_packet(
    ...
    ipv4_hdr->total_length   = RTE_CPU_TO_BE_16(tcp_len + sizeof(struct rte_ipv4_hdr));
    ipv4_hdr->src_addr = ip_src_addr;
    ipv4_hdr->dst_addr = ip_dst_addr;
    ...
    return (uint8_t*)(ipv4_hdr+1);
}
void send_ip_raw(struct rte_mbuf *pkt,char data[],rte_be32_t src_addr, rte_be32_t dst_addr, struct rte_ether_addr eth_src_addr, struct rte_ether_addr eth_dst_addr){
